...
export interface PayablePersonData {
  personId: string;
  personName: string;
  personMobile?: string;
...
interface PayablePersonsModalProps {
  isOpen: boolean;
  onClose: () => void;
  payablePersons: PayablePersonData[];
  onViewPersonDetails: (person: Person) => void;
  persons: Person[]; 
}

const PayablePersonsModal: React.FC<PayablePersonsModalProps> = ({
  isOpen,
  onClose,
  payablePersons,
  onViewPersonDetails,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="payable-persons-modal-title"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-200">
          <h2 id="payable-persons-modal-title" className="text-xl font-semibold text-slate-800">
            {BN_UI_TEXT.PAYABLE_PERSONS_MODAL_TITLE}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-grow custom-scrollbar-modal pr-1">
          {payablePersons.length === 0 ? (
            <p className="text-slate-500 text-center py-8">{BN_UI_TEXT.NO_PERSONS_WITH_PAYABLES}</p>
          ) : (
            <ul className="space-y-3">
              {payablePersons.map((item) => (
                <li key={item.personId} className="p-3 bg-slate-50 rounded-lg border border-slate-200 hover:shadow-md transition-shadow duration-150">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-md font-semibold text-red-700">{item.personName}</h3>
                      {item.personMobile && (
                      ...
                      <p className="text-sm text-slate-600 mt-0.5">
                        {BN_UI_TEXT.TOTAL_PAYABLE_TO_PERSON_LABEL}{' '}
                        <span className="font-bold text-red-600">{formatCurrency(item.totalPayableAmount)}</span>
                      </p>
                    </div>
                    <button
                      ...
                      title={BN_UI_TEXT.VIEW_DETAILS_FOR_PERSON_PAYABLE_TOOLTIP.replace('{personName}', item.personName)}
                      aria-label={BN_UI_TEXT.VIEW_DETAILS_FOR_PERSON_PAYABLE_TOOLTIP.replace('{personName}', item.personName)}
                    >
...
export default PayablePersonsModal;